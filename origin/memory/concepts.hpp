...
template <typename A> struct get_allocate_result
{
private:
  template <typename X> static auto check (X &x) -> decltype (x.allocate (1));
...
template <typename A> struct get_deallocate_result
{
private:
  template <typename X>
...
template <typename A>
using Allocate_result = typename memory_impl::get_allocate_result<A>::type;
...
template <typename A>
using Deallocate_result = typename memory_impl::get_deallocate_result<A>::type;
