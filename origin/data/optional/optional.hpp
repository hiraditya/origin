...
public:
  ...
  template <typename Arg, typename = Requires<Assignable<T, Arg> ()> >
...
template <typename T> inline optional<T>::optional () : init (false) {}

// Move semantics
template <typename T>
inline optional<T>::optional (optional &&x)
    : init (x.init)
{
  if (x.init)
...
template <typename T>
inline auto optional<T>::operator= (optional &&x) -> optional &
{
  if (init)
    {
      if (x.init)
...
// Copy semantics
template <typename T>
inline optional<T>::optional (optional const &x)
    : init (x.init)
{
  if (x.init)
    new (ptr ()) T (x.get ());
...
template <typename T>
inline auto optional<T>::operator= (optional const &x) -> optional &
{
  if (init)
    {
      if (x.init)
...
inline auto optional<T>::operator= (Arg &&arg) -> optional &
{
...
template <typename T>
inline auto optional<T>::operator= (std::nullptr_t) -> optional &
{
  clear ();
  return *this;
}
...
// Destruction
template <typename T> inline optional<T>::~optional ()
{
  if (init)
    ptr ()->~T ();
}
