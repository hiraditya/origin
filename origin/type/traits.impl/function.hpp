...
struct invoke
{
  ...
  static auto fn (P p, C &&c) -> decltype (std::forward<C> (c).*p);
...
template <typename... Args> struct result_of<void(Args...)>
{
  using type = subst_failure;
};
...
// functor.
template <typename T> struct get_call_operator_type
{
private:
  template <typename X> static auto check (X) -> decltype (&X::operator());
...
struct function_arity : std::integral_constant<int, -1>
{
};
...
template <typename F, bool = Class<F> ()> struct get_parameter_types
{
  using type = subst_failure;
};
...
template <typename F, bool = Class<F> ()> struct get_result_type
{
  using type = subst_failure;
};
