...
template <typename Prop, typename... Args>
void
context::check (Prop prop, Args &&... args)
{
...
template <typename Prop, typename... Args>
void
context::quick_check (Prop prop, Args &&... args)
{
...
void
log_args (Stream &os, T &&value, Args &&... args)
{
  log_arg (os, std::forward<T> (value));
  os << ',' << ' ';
...
//
template <typename T>
auto quantify_over () -> decltype (quantify_over (default_pattern<T> ()))
{
