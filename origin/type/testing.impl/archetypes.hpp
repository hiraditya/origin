...
template <typename Arch, typename T> struct default_constructible_archetype
{
  static_assert (Default_constructible<T> (), "");
};

template <typename Arch, typename T> struct move_constructible_archetype
{
  static_assert (Move_constructible<T> (), "");
};

template <typename Arch, typename T> struct copy_constructible_archetype
{
...
struct equality_comparable_archetype
{
  ...
  friend bool
  operator== (const Arch &a, const U &b)
  {
    return a.value == b;
  }
  friend bool
  operator== (const U &a, const Arch &b)
  {
    return a == b.value;
  }
  friend bool
...
struct equality_comparable_archetype<Arch, T, default_t>
{
  ...
  friend bool
  operator== (const Arch &a, const Arch &b)
  {
...
struct totally_ordered_archetype<Arch, T, default_t>
    : equality_comparable_archetype<Arch, T>, weakly_ordered_archetype<Arch, T>
{
...
// Semiregular
template <typename Arch, typename T>
struct semiregulr_archetype : copyable_archetype<Arch, T>,
...
// Regular
template <typename Arch, typename T>
struct regular_archetype : semiregulr_archetype<Arch, T>,
...
class function_archetype : public copy_constructible_archetype<Arch, F>
{
  ...
  self ()
  {
    return *static_cast<Arch *> (this);
  }
  const Arch &
  self () const
  {
    return *static_cast<const Arch *> (this);
  }
...
public:
  bool operator() (Args... args) const { return self ().value (args...); }
};
...
struct use_movable
{
  ...
  template <typename Arch, typename T> using impl = movable_archetype<Arch, T>;
};
...
template <typename U = default_t> struct use_equality_comparable
{
  using refines = std::tuple<>;
...
template <> struct use_equality_comparable<default_t>
{
  using refines = std::tuple<>;
...
template <typename U = default_t> struct use_weakly_ordered
{
  using refines = std::tuple<>;
...
template <> struct use_weakly_ordered<default_t>
{
  using refines = std::tuple<>;
...
template <typename U = default_t> struct use_totally_ordered
{
  using refines
      = std::tuple<use_equality_comparable<U>, use_weakly_ordered<U> >;
...
template <> struct use_totally_ordered<default_t>
{
  using refines = std::tuple<use_equality_comparable<>, use_weakly_ordered<> >;
...
struct use_regular
{
  using refines = std::tuple<use_default_constructible, use_copyable,
                             use_equality_comparable<> >;

  template <typename Arch, typename T> using impl = regular_archetype<Arch, T>;
};

// Function (selector)
template <typename... Args> struct use_function
{
...
// Predicate (selector)
template <typename... Args> struct use_predicate
{
  using refines = std::tuple<use_copy_constructible>;
...
// Relation (selector)
template <typename T, typename U = T> struct use_relation
{
  using refines = std::tuple<use_copy_constructible>;
...
template <typename T> struct use_output_iterator
{
...
struct has_selector<Sel, T, Args...>
    : boolean_constant<Same<Sel, T> () ? true
...
template <typename Sel> struct has_selector<Sel> : std::false_type
{
};
...
template <typename Sel, typename T>
struct inspect_selector : inspect_refinements<Sel, typename T::refines>
{
};
