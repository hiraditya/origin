...
struct crosstype_equality
{
  ...
  operator() (const T &t, const U &u) const
  {
    ...
    return (t == u) == (C (t) == C (u)) && (u == t) == (C (u) == C (t));
  }
};
...
//
//
template <typename T, typename U = T> struct weakly_ordered
{
...
template <typename T> struct weakly_ordered<T, T>
{
...
// to ==.
//
template <typename T, typename U = T> struct totally_ordered
{
...
template <typename T> struct totally_ordered<T, T>
{
...
// Move Constructuble
//
template <typename T> struct move_constructible
{
  template <typename Tv>
  void
  operator() (Tv &&tvar) const
...
// Semiregular
//
template <typename T> struct semiregular
{
