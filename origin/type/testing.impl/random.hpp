...
template <typename T> class singleton_generator
{
public:
  singleton_generator (const T &val) : value (val) {}
...
template <typename T>
singleton_generator<T>
single (const T &value)
{
  return { value };
}

template <typename T> struct single_value_distribution
{
...
{
  single_value_distribution<std::nullptr_t>
  operator() () const
  {
...
template <typename T> struct default_int_distribution
{
...
template <typename T> struct default_float_distribution
{
...
// concept.
template <typename T> subst_failure deduce_default_pattern (...);
...
// method above.
template <typename T> struct default_pattern_traits
{
  using type = decltype (deduce_default_pattern (testing::adl_t{},
                                                 std::declval<T> ()));
};

// Unwrap associations to reference types.
template <typename T> struct default_pattern_traits<T &>
{
...
template <typename T> struct default_pattern_traits<const T>
{
  using type = typename default_pattern_traits<T>::type;
};
...
template <> struct default_pattern_traits<bool>
{
...
// returns nulltprs.
template <> struct default_pattern_traits<std::nullptr_t>
{
