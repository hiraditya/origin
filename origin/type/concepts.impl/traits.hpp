...
struct is_equality_comparable<T, T>
    : boolean_constant<Has_equal<T> () && Boolean<Equal_result<T> > ()
                       && Has_not_equal<T> ()
...
struct is_weakly_ordered
    : boolean_constant<Common<T, U> () && Totally_ordered<T> ()
                       && Totally_ordered<T> ()
                       && Totally_ordered<Common_type<T, U> > ()
                       && Has_less<T, U> () && Boolean<Less_result<T, U> > ()
                       && Has_less<U, T> () && Boolean<Less_result<U, T> > ()
                       && Has_greater<T, U> ()
                       && Boolean<Greater_result<T, U> > ()
                       && Has_greater<U, T> ()
                       && Boolean<Greater_result<U, T> > ()
                       && Has_less_equal<T, U> ()
                       && Boolean<Less_equal_result<T, U> > ()
                       && Has_less_equal<U, T> ()
                       && Boolean<Less_equal_result<U, T> > ()
                       && Has_greater_equal<T, U> ()
                       && Boolean<Greater_equal_result<T, U> > ()
                       && Has_greater_equal<U, T> ()
                       && Boolean<Greater_equal_result<U, T> > ()>
{
};
...
struct is_weakly_ordered<T, T>
    : boolean_constant<Has_less<T> () && Boolean<Less_result<T> > ()
                       && Has_greater<T> () && Boolean<Greater_result<T> > ()
                       && Has_less_equal<T> ()
                       && Boolean<Less_equal_result<T> > ()
                       && Has_greater_equal<T> ()
...
struct is_weakly_ordered_noverify
    : boolean_constant<Has_less<T> () && Has_greater<T> ()
                       && Has_less_equal<T> () && Has_greater_equal<T> ()>
{
};
...
struct is_relation
    : boolean_constant<Common<T, U> () && Relation<R, T> ()
                       && Relation<R, U> ()
...
struct is_relation<R, T, T> : boolean_constant<Predicate<R, T, T> ()>
{
};
...
template <typename S, typename T> struct get_input_stream_result
{
private:
...
struct is_input_streamable
    : boolean_constant<Subst_succeeded<
          typename get_input_stream_result<S, T>::type> ()>
{
};

template <typename T>
struct is_input_streamable<T, default_t> : is_input_streamable<std::istream, T>
{
};

// Safely deduce the result type of the expression s << x, for any Stream s.
template <typename S, typename T> struct get_output_stream_result
{
private:
  template <typename X, typename Y>
...
public:
  using type
      = decltype (check (std::declval<S &> (), std::declval<T const &> ()));
};
...
struct is_output_streamable
    : boolean_constant<Subst_succeeded<
          typename get_output_stream_result<S, T>::type> ()>
{
};
