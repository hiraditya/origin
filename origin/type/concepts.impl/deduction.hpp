...
auto deduce_value_type (default_t, const T &)
    -> Requires<Has_dereference<T> (),
                Structural_type<Dereference_result<T> > >;
...
template <typename T> struct get_deduced_value_type<T, true>
{
  using type = typename get_associated_value_type<T>::type;
};

template <typename T> struct get_deduced_value_type<T, false>
{
...
template <typename T> struct get_deduced_size_type<T, true>
{
  using type = typename get_associated_size_type<T>::type;
};

template <typename T> struct get_deduced_size_type<T, false>
{
...
template <typename T> std::ptrdiff_t deduce_difference_type (default_t, T *);
...
template <typename T> struct get_deduced_difference_type<T, true>
{
...
template <typename T> struct get_deduced_difference_type<T, false>
{
  using type
...
template <typename T, bool = Has_associated_reference<T> (),
...
template <typename T> struct get_deduced_reference<T, true, true>
{
  using type = If<Const<Remove_reference<T> > (),
                  Associated_const_reference<T>, Associated_reference<T> >;
};

// Iterators typically define an associated reference type, but not a const
// reference.
template <typename T> struct get_deduced_reference<T, true, false>
{
...
template <typename T> struct get_deduced_reference<T, false, true>
{
...
// get an answer.
template <typename T> struct get_deduced_reference<T, false, false>
{
...
template <typename T, bool = Has_associated_pointer<T> (),
...
template <typename T> struct get_deduced_pointer<T, true, true>
{
  using type = If<Const<Remove_reference<T> > (), Associated_const_pointer<T>,
                  Associated_pointer<T> >;
};

// Iterators typically define an associated pointer, but not a const pointer.
template <typename T> struct get_deduced_pointer<T, true, false>
{
...
template <typename T> struct get_deduced_pointer<T, false, true>
{
...
// answer.
template <typename T> struct get_deduced_pointer<T, false, false>
{
