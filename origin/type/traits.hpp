...
template <bool B> using boolean_constant = std::integral_constant<bool, B>;
...
template <typename T> struct Expand
{
};
...
template <std::size_t N, typename... Args> struct selection;

template <std::size_t N, typename T, typename... Args>
struct selection<N, T, Args...> : selection<N - 1, Args...>
{
};

template <typename T, typename... Args> struct selection<0, T, Args...>
{
  using type = T;
};
...
// in those cases.
template <typename T> struct common_type<T, T>
{
  using type = typename std::remove_const<
...
template <template <typename...> class Template, typename... Args>
struct common_type<Expand<Template<Args...> > > : origin::common_type<Args...>
{
};
...
template <typename T> using Add_pointer = typename std::add_pointer<T>::type;
...
template <typename C> using Result_of = typename type_impl::result_of<C>::type;
...
// WARNING: This property cannot be deduced for polymorphic functors.
template <typename F>
using Domain = If<Homogeneous_function<F> (),
...
// WARNING: This property cannot be deduced for polymorphic functors.
template <typename F>
using Codomain = Structural_type<typename type_impl::get_result_type<F>::type>;
...
Qualified ()
{
  return Const<T> () || Volatile<T> ();
}

// An alias to const-qualified T (i.e., const T).
template <typename T> using Add_const = typename std::add_const<T>::type;

// An alias to volatile-qualified T (i.e., volatile T).
template <typename T> using Add_volatile = typename std::add_volatile<T>::type;

// An alias to const volatile qualified T (i.e., const volatile T).
template <typename T> using Add_cv = typename std::add_cv<T>::type;

// An alias to U if T has type const U, otherwise T.
template <typename T> using Remove_const = typename std::remove_const<T>::type;
...
template <typename T> using Remove_cv = typename std::remove_cv<T>::type;
...
template <typename T, typename U> struct get_plus_result
{
private:
  template <typename X, typename Y>
...
// Deduce the result type of the expression t - u.
template <typename T, typename U> struct get_minus_result
{
private:
  template <typename X, typename Y>
  static auto check (X &&x, Y &&y) -> decltype (x - y);
...
// Deduce the result type of the expression t * u.
template <typename T, typename U> struct get_multiplies_result
{
private:
  template <typename X, typename Y>
  static auto check (X &&x, Y &&y) -> decltype (x *y);
...
// Deduce the result type of the expression t / u.
template <typename T, typename U> struct get_divides_result
{
private:
  template <typename X, typename Y>
  static auto check (X &&x, Y &&y) -> decltype (x / y);
...
// Deduce the result type of the expression t % u.
template <typename T, typename U> struct get_modulus_result
{
private:
  template <typename X, typename Y>
  static auto check (X &&x, Y &&y) -> decltype (x % y);
...
template <typename T> struct get_unary_plus_result
{
private:
  template <typename X> static auto check (X &&a) -> decltype (+a);
...
// Deduce the result of -t.
template <typename T> struct get_unary_minus_result
{
private:
  template <typename X> static auto check (X &&a) -> decltype (-a);
...
{
// Deduce the result of ++t;
template <typename T> struct get_pre_increment_result
{
private:
  template <typename X> static auto check (X &x) -> decltype (++x);
  static subst_failure check (...);
...
// Deduce the result of t++;
template <typename T> struct get_post_increment_result
{
private:
  template <typename X> static auto check (X &x) -> decltype (x++);
  static subst_failure check (...);
...
// Deduce the result of --t;
template <typename T> struct get_pre_decrement_result
{
private:
  template <typename X> static auto check (X &x) -> decltype (--x);
  static subst_failure check (...);
...
// Deduce the result of t--;
template <typename T> struct get_post_decrement_result
{
private:
  template <typename X> static auto check (X &x) -> decltype (x--);
  static subst_failure check (...);
...
{
// Deduce the result type of the expression t && u.
template <typename T, typename U> struct get_and_result
{
private:
  template <typename X, typename Y>
  static auto check (X &&x, Y &&y) -> decltype (x &&y);
...
// Deduce the result type of the expression t || u.
template <typename T, typename U> struct get_or_result
{
private:
  template <typename X, typename Y>
  static auto check (X &&x, Y &&y) -> decltype (x || y);
...
// Deduce the result of !t.
template <typename T> struct get_not_result
{
private:
  template <typename X> static auto check (X &&a) -> decltype (!a);
...
{
// Deduce the result type of the expression t & u.
template <typename T, typename U> struct get_bit_and_result
{
private:
  template <typename X, typename Y>
  static auto check (X &&x, Y &&y) -> decltype (x &y);
...
// Deduce the result type of the expression t | u.
template <typename T, typename U> struct get_bit_or_result
{
private:
  template <typename X, typename Y>
  static auto check (X &&x, Y &&y) -> decltype (x | y);
...
// Deduce the result type of the expression t ^ u.
template <typename T, typename U> struct get_bit_xor_result
{
private:
  template <typename X, typename Y>
  static auto check (X &&x, Y &&y) -> decltype (x ^ y);
...
// Deduce the result type of the expression t << u.
template <typename T, typename U> struct get_left_shift_result
{
private:
  template <typename X, typename Y>
  static auto check (X &&x, Y &&y) -> decltype (x << y);
...
// Deduce the result type of the expression t >> u.
template <typename T, typename U> struct get_right_shift_result
{
private:
  template <typename X, typename Y>
  static auto check (X &&x, Y &&y) -> decltype (x >> y);
...
// Deduce the result of ~t.
template <typename T> struct get_complement_result
{
private:
  template <typename X> static auto check (X &&a) -> decltype (~a);
...
// An alias for the result type of the expression t << u.
template <typename T, typename U = T>
using Left_shift_result =
    typename type_impl::get_left_shift_result<T, U>::type;
...
// An alias for the result type of the expression t >> u.
template <typename T, typename U = T>
using Right_shift_result =
    typename type_impl::get_right_shift_result<T, U>::type;
...
{
// Deduce the result type of the expression t &= u.
template <typename T, typename U> struct get_bit_and_assign_result
{
private:
  template <typename X, typename Y>
  static auto check (X &x, Y &&y) -> decltype (x &= y);
...
// Deduce the result type of the expression t |= u.
template <typename T, typename U> struct get_bit_or_assign_result
{
private:
  template <typename X, typename Y>
  static auto check (X &x, Y &&y) -> decltype (x |= y);
...
// Deduce the result type of the expression t ^= u.
template <typename T, typename U> struct get_bit_xor_assign_result
{
private:
  template <typename X, typename Y>
  static auto check (X &x, Y &&y) -> decltype (x ^= y);
...
// Deduce the result type of the expression t <<= u.
template <typename T, typename U> struct get_left_shift_assign_result
{
private:
  template <typename X, typename Y>
  static auto check (X &x, Y &&y) -> decltype (x <<= y);
...
// Deduce the result type of the expression t >>= u.
template <typename T, typename U> struct get_right_shift_assign_result
{
private:
  template <typename X, typename Y>
  static auto check (X &x, Y &&y) -> decltype (x >>= y);
...
template <typename T> struct get_address_result
{
private:
  template <typename X> static auto check (X &&a) -> decltype (&a);
...
// Deduce the result of the expression *t.
template <typename T> struct get_dereference_result
{
private:
  template <typename X> static auto check (X &&a) -> decltype (*a);
...
template <typename T, typename U> struct get_subscript_result
{
private:
  template <typename X, typename Y>
  static auto check (X &&x, Y &&y) -> decltype (x[y]);
...
template <typename T>
constexpr bool
Has_associated_difference_type ()
{
...
template <typename T>
constexpr bool
Has_associated_const_reference ()
{
...
template <typename T>
using Associated_pointer = typename type_impl::get_associated_pointer<T>::type;

// Return true if T::cosnt_reference is a valid type name.
template <typename T>
constexpr bool
Has_associated_pointer ()
{
