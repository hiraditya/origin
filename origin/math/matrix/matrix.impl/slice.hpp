...
template <std::size_t N> struct matrix_slice
{
  ...
  matrix_slice (const matrix_slice<M> &s, std::integral_constant<T, D>,
                std::size_t n);
  ...
  // Return the offset
  template <typename R> std::size_t offset (R &&range) const;

  // Returns a slice describing the nth row in the Dth dimension.
  template <std::size_t D> matrix_slice<N - 1> row (std::size_t n) const;
...
private:
  ...
  template <std::size_t M> std::size_t do_slice (const matrix_slice<M> &);
...
matrix_slice<N>::offset (R &&range) const
{
  ...
  return start
         + std::inner_product (strides, strides + N, begin (range), zero);
}
...
matrix_slice<N>::matrix_slice (const matrix_slice<M> &s,
                               std::integral_constant<T, D>, std::size_t n)
...
// Compute the extents and strides for the specified slice in the dim
// dimension.
...
// element is effectively computed as the dot product of the starting offsets
// of
// each slice argument with the strides of this slice.
//
// All of the heavy lifting is done in do_slice_dim.
//
// TODO: It is possible to reduce the dimensionality of the resulting slice
// when
...
template <std::size_t M, typename T, typename... Args>
inline std::size_t
matrix_slice<N>::do_slice (const matrix_slice<M> &desc, const T &s,
                           const Args &... args)
{
...
{
  return a.start == b.start && std::equal (a.extents, a.extents + N, b.extents)
