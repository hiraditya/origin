...
public:
  ...
  template <typename... Dims> explicit matrix (Dims... dims);
  ...
  template <typename U> matrix (std::initializer_list<U> list) = delete;
  ...
  // Matrix arithmetic
  template <typename M> matrix &operator+= (const M &m);
...
inline matrix<T, N>::matrix (const M &x)
...
template <typename T, std::size_t N>
inline matrix<T, N>::matrix (const matrix_slice<N> &slice)
...
inline matrix<T, N>::matrix (Dims... dims)
...
template <typename T, std::size_t N>
template <typename... Args>
inline Requires<matrix_impl::Index_sequence<Args...> (), T &> matrix<T, N>::
operator() (Args... args)
{
