...
public:
  ...
  // matrix_ref.
  template <typename U> matrix_ref (const matrix_ref<U, N> &x);

  template <typename U> matrix_ref &operator= (const matrix_ref<U, N> &x);
  ...
  template <typename M, typename F> matrix_ref &apply (const M &m, F f);
  ...
  // Matrix arithmetic
  template <typename M> matrix_ref &operator+= (const M &m);
...
inline matrix_ref<T, N>::matrix_ref (matrix_ref &&x)
...
template <typename T, std::size_t N>
inline matrix_ref<T, N>::matrix_ref (const matrix_ref &x)
    : desc (x.desc), ptr (x.ptr)
{
}
...
template <typename T, std::size_t N>
inline matrix_ref<T, N>::matrix_ref (matrix<value_type, N> &x)
    : desc (x.descriptor ()), ptr (x.data ())
{
}

template <typename T, std::size_t N>
inline matrix_ref<T, N>::matrix_ref (const matrix<value_type, N> &x)
    : desc (x.descriptor ()), ptr (x.data ())
{
}
...
template <typename T, std::size_t N>
template <typename U>
inline matrix_ref<T, N>::matrix_ref (const matrix_ref<U, N> &x)
...
template <typename T, std::size_t N>
inline matrix_ref<T, N>::matrix_ref (const matrix_slice<N> &s, T *p)
    : desc (s), ptr (p)
{
}
...
template <typename T, std::size_t N>
template <typename... Args>
inline Requires<matrix_impl::Slice_sequence<Args...> (),
