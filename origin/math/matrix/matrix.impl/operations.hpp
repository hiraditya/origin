...
template <typename M> inline auto rows (const M &m) -> decltype (m.extent (0))
{
...
template <typename M> inline auto cols (const M &m) -> decltype (m.extent (1))
{
...
//
template <typename T, std::size_t N>
inline matrix<T, N> operator* (const matrix<T, N> &x, const T &n)
{
  matrix<T, N> result = x;
  return result *= n;
}
...
template <typename T, std::size_t N>
inline matrix<T, N> operator* (const T &n, const matrix<T, N> &x)
{
  matrix<T, N> result = x;
  return result *= n;
}
...
template <typename T, std::size_t N>
inline matrix<T, N> operator* (const T &n, const matrix_ref<T, N> &x)
{
  matrix<T, N> result = x;
  return result *= n;
}
...
template <typename T>
inline matrix<T, 2> operator* (const matrix<T, 2> &a,
                               const matrix_ref<T, 2> &b)
{
  matrix<T, 2> result (a.dims ());
  matrix_product (a, b, result);
...
template <typename M, std::size_t D = M::order> struct pretty_matrix
{
...
pretty (const M &m)
{
  return pretty_matrix<M>{ m };
}

template <typename M>
Requires<Matrix<M> (), pretty_matrix<M> >
pretty (const M &m, std::size_t d)
{
  return pretty_matrix<M>{ m, d };
}
