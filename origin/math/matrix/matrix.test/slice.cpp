...
bool
eq (R &&range, std::initializer_list<T> list)
{
  return equal (begin (range), end (range), begin (list));
}

template <std::size_t N> struct slice_enumerator
{
  ...
  bool
  operator== (const slice_enumerator &x) const
  {
...
template <std::size_t N> struct slice_enumerator_range
{
  slice_enumerator_range (const matrix_slice<N> &s) : slice (s) {}
...
enumerate (const matrix_slice<N> &slice)
{
  return { slice };
}
