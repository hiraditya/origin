...
template <typename T> struct get_std_begin_result
{
private:
  template <typename X> static auto check (X &&x) -> decltype (std::begin (x));
...
template <typename T> struct get_std_end_result
{
private:
  template <typename X> static auto check (X &&x) -> decltype (std::end (x));
  static subst_failure check (...);
...
template <typename T> struct get_adl_begin_result
{
private:
  template <typename X> static auto check (X &&x) -> decltype (begin (x));
  static subst_failure check (...);
...
template <typename T> struct get_adl_end_result
{
private:
  template <typename X> static auto check (X &&x) -> decltype (end (x));
  static subst_failure check (...);
...
//    begin(r);
//
template <typename T, bool = Has_std_begin<T> ()> struct get_begin_result;

template <typename T> struct get_begin_result<T, true>
{
  using type = typename get_std_begin_result<T>::type;
};

template <typename T> struct get_begin_result<T, false>
{
...
//    end(r);
//
template <typename T, bool = Has_std_end<T> ()> struct get_end_result;

template <typename T> struct get_end_result<T, true>
{
  using type = typename get_std_end_result<T>::type;
};

template <typename T> struct get_end_result<T, false>
{
  using type = typename get_adl_end_result<T>::type;
};

// Safely deduce the result of the expression std::swap(r, s).
template <typename T> struct get_std_swap_result
{
private:
  template <typename X>
...
// non-std namespace and the type of r is defined in that namespace.
template <typename T> struct get_adl_swap_result
{
private:
  template <typename X>
...
//    swap(r, s);
//
template <typename T, bool = Has_std_swap<T> ()> struct get_swap_result;

template <typename T> struct get_swap_result<T, true>
{
  using type = typename get_std_swap_result<T>::type;
};

template <typename T> struct get_swap_result<T, false>
{
...
template <typename T> struct get_associated_iterator_category
{
private:
  template <typename X> static typename X::iterator_category check (const X &);
...
template <typename T> struct iterator_category_traits
{
  using type = typename get_associated_iterator_category<T>::type;
};

// Specialization for pointers.
template <typename T> struct iterator_category_traits<T *>
{
...
// reference types.
template <typename T>
struct iterator_category_traits<T &> : iterator_category_traits<T>
{
};
