...
struct sorted_range
{
  ...
  bool
  operator() (const R &range, C comp) const
  {
    using std::begin;
    using std::end;
    auto first = begin (range);
...
// iterator-based algorithm. The property can check the following properties:
//
//    is_sorted_until(range)       <=> is_sorted_until(begin(range),
//    end(range))
//    is_sorted_utnil(range, comp) <=> is_sorted_until(begin(range),
//    end(range), comp)
struct partially_sorted_range
{
  ...
  operator() (const R &range, C comp) const
  {
    ...
    auto first = begin (range);
    auto last = end (range);
    return is_sorted_until (range, comp)
...
main ()
{
  ...
  assert (is_sorted_until (v1, lt)
          == is_sorted_until (v1.begin (), v1.end (), lt));
}
