...
struct iterator_move_assignment
{
  ...
  bool
  operator() (R &&range, const T &a, const T &b)
  {
    using std::begin;
    using std::end;
...
struct iterator_copy_assignment
{
  ...
  template <typename I, typename T>
  bool
  operator() (I first, I last, const T &x) const
  {
    if (first != last)
      {
        // Note that we can't just write (*first = x) == x because the result
...
// Readable Range
//
template <typename R> struct readable_range
{
...
template <typename R, typename T> struct writable_range
{
  template <typename Rv, typename Tv>
  void
  operator() (Rv &&rvar, Tv &&tvar) const
  {
...
template <typename R, typename T> struct writable_range<R, T &&>
{
  template <typename Rv, typename Tv>
  void
  operator() (Rv &&rvar, Tv &&tvar) const
...
template <typename R> struct permutable_range
{
  template <typename Rv>
  void
  operator() (Rv &&rvar)
...
template <typename R> struct mutable_range
{
  template <typename Rv>
  void
  operator() (Rv &&rvar)
...
template <typename R> struct weakly_incrementable_range
{
  template <typename Rv>
  void
  operator() (Rv &&rvar) const
  {
...
// Incrementable Range
//
template <typename R> struct incrementable_range
{
  template <typename Rv>
  void
  operator() (Rv &&rvar) const
...
template <typename R>
void
check_incrementable_range (R &&range)
{
  auto rvar = quantify_over_range (range);
