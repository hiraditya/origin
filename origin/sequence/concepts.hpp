...
Readable ()
{
  ...
  return Has_value_type<I> () && Has_dereference<I> ()
...
Permutable ()
{
  return Readable<I> () && Movable<Value_type<I> > ()
...
Mutable ()
{
  return Permutable<I> () && Copyable<Value_type<I> > ()
...
template <typename I>
constexpr bool
Pseudo_incrementable ()
{
  return Copyable<I> ()

         // ++i return I&
         && Has_pre_increment<I> () && Same<Pre_increment_result<I>, I &> ();
}
...
Weakly_incrementable ()
{
  return Copyable<I> ()
         ...
         && Has_difference_type<I> () && Signed<Difference_type<I> > ()
...
Incrementable ()
{
  return Weakly_incrementable<I> ()
         ...
         // i++ returns I
         && Has_post_increment<I> () && Same<Post_increment_result<I>, I> ()
...
Decrementable ()
{
  return Incrementable<I> ()
         ...
         // --i returns I&
         && Has_pre_decrement<I> () && Same<Pre_decrement_result<I>, I &> ()

         // i-- returns I
         && Has_post_decrement<I> () && Same<Post_decrement_result<I>, I> ();
}
...
Iterator ()
{
  return Weakly_incrementable<I> () && Has_dereference<I> ()
...
Input_iterator ()
{
  return Readable<I> () && Weakly_incrementable<I> ()
         && Equality_comparable<I> ()
...
Output_iterator ()
{
  return Pseudo_incrementable<I> () && Writable<I, T> ();
}
...
{
  return Readable<I> () && Decrementable<I> ()
...
Random_access_iterator ()
{
  return Readable<I> () && Decrementable<I> ()
         ...
         && Has_minus<I> () && Same<Minus_result<I>, Difference_type<I> > ()

         // i[n] returns Reference_of<I>
         && Has_subscript<I, Difference_type<I> > ()
         && Same<Subscript_result<I, Difference_type<I> >,
                 Dereference_result<I> > ()
...
template <typename R> using Iterator_of = Begin_result<R>;
...
template <typename T>
constexpr bool
has_input_iterator_property ()
{
  return Input_iterator<T> () || Forward_iterator<T> ()
         || Bidirectional_iterator<T> () || Random_access_iterator<T> ();
}

// Returns true if T has at least forward iterators
template <typename T>
constexpr bool
has_forward_iterator_property ()
{
  return Forward_iterator<T> () || Bidirectional_iterator<T> ()
...
template <typename T>
constexpr bool
has_bidirectional_iterator_property ()
{
  return Bidirectional_iterator<T> () || Random_access_iterator<T> ();
}

// Returns true if T has random access iterators
template <typename T>
constexpr bool
has_random_access_iterator_property ()
{
...
is_STL_container ()
{
  return has_input_iterator_property<Iterator_of<T> > () && Has_begin<T> ()
         && Has_end<T> () && Has_swap<T> () && Has_member_empty<T> ()
         && Has_member_max_size<T> () && Has_member_size<T> ()
...
is_STL_sequence ()
{
  return is_STL_container<T> () && Has_member_clear<T> ()
         && Has_member_erase<T, Iterator_of<T> > () && Has_member_front<T> ()
...
template <typename T>
constexpr bool
is_STL_forward_container ()
{
  return is_STL_container<T> () && Totally_ordered<Value_type<T> > ();
}

// TODO: Does having bidirectional iterator guarantee backwards iteration?
template <typename T>
constexpr bool
is_STL_reversible_container ()
{
...
template <typename T>
constexpr bool
is_STL_random_access_container ()
{
...
template <typename T>
constexpr bool
is_STL_associative_container ()
{
  return is_STL_forward_container<T> () && Default_constructible<T> ()
         && Has_associated_key_type<T> () && Has_member_clear<T> ()
...
template <typename T>
constexpr bool
is_STL_hashed_associative_container ()
{
  return is_STL_associative_container<T> () && Has_associated_hasher<T> ()
         && Has_associated_key_equal<T> () && Has_member_key_eq<T> ()
