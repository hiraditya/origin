...
{
  return g.out_degree (v) == a[0] && g.in_degree (v) == a[1];
}
...
template <typename G>
G
build_reflexive_bidi_clique (int n)
{
  G g = build_n_graph<G> (n);
  int x = 0;
  for (int i = 0; i < n; ++i)
...
check_remove_first_multi_edge ()
{
  ...
  // cout << io::edge_list(g) << '\n';
  assert (
      has_degrees (g, 0, { 3, 3, 6 })); // a--a, a--b, a--c | a--a, b--a, c--a

  g.remove_edge (0, 0);
  // cout << io::edge_list(g) << '\n';
  assert (has_degrees (g, 0, { 2, 2, 4 })); // a--b, a--c | b--a, c--a
  assert (has_degrees (
      g, 1, { 4, 4, 8 })); // b--a, b--b, b--b, b--c | a--b, b--b, b--b, c--b
  ...
  assert (has_degrees (
      g, 1, { 4, 3, 7 })); // b--a, b--b, b--b, b--c | b--b, b--b, c--b

  g.remove_edge (1, 0);
  // cout << io::edge_list(g) << '\n';
  assert (has_degrees (g, 0, { 1, 1, 2 })); // a--c | c--a
  assert (
      has_degrees (g, 1, { 3, 3, 6 })); // b--b, b--b, b--c | b--b, b--b, c--b
...
check_remove_multi_edge ()
{
  ...
  if (Directed_graph<G> ())
  ...
  else
    {
      assert (has_degrees (g, 0, { 1, 1, 2 })); // a--c | c--a
      assert (has_degrees (
          g, 1, { 3, 3, 6 })); // b--b, b--b, b--c | b--b, b--b, c--b
    }
  ...
  if (Directed_graph<G> ())
    {
      ...
      assert (has_degrees (
          g, 2, { 4, 3, 7 })); // c--a, c--b c--c, c--c | b--c, c--c, c--c
    }
  else
    {
      assert (has_degrees (g, 0, { 0, 0, 0 })); // empty | empty
      assert (has_degrees (
          g, 2, { 3, 3, 6 })); // c--b, c--c, c--c | b--c, c--c, c--c
    }
