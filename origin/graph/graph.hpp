...
template <typename G>
inline auto vertices (const G &g) -> decltype (g.vertices ())
{
  return g.vertices ();
}

// Returns a range over the edges of a graph.
template <typename G> inline auto edges (const G &g) -> decltype (g.edges ())
{
  return g.edges ();
}
...
template <typename G>
inline bool
is_isolated (const G &g, Vertex<G> v)
{
  return g.degree (v) == 0;
}
...
template <typename G>
inline bool
is_source (const G &g, Edge<G> e, Vertex<G> v)
{
  return source (g, e) == v;
}
...
template <typename G>
inline bool
is_target (const G &g, Edge<G> e, Vertex<G> v)
{
  return target (g, e) == v;
}
...
template <typename G>
inline bool
is_loop (const G &g, Edge<G> e)
{
  return source (g, e) == target (g, e);
}
...
template <typename G> struct has_target
{
  has_target (const G &g, Vertex<G> v) : g (g), v (v) {}
...
template <typename G> struct has_source
{
  has_source (const G &g, Vertex<G> v) : g (g), v (v) {}
...
template <typename G> struct has_endpoint
{
...
template <typename G> struct has_endpoints
{
  has_endpoints (const G &g, Vertex<G> u, Vertex<G> v) : g (g), u (u), v (v) {}
...
template <typename G> struct is_looped
{
  is_looped (const G &g, Vertex<G> v) : g (g), v (v) {}
