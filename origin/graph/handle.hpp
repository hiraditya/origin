...
inline handle::handle (std::size_t n) : value (n) {}

inline handle::operator bool () const { return value != npos; }
...
//
template <typename E> struct multi_edge_handle
{
  using handle_type = E;

  multi_edge_handle () : value (vertex_handle::npos, vertex_handle::npos, E{})
  {
  }

  multi_edge_handle (vertex_handle s, vertex_handle t, E e) : value (s, t, e)
  {
  }
...
namespace std
{
template <> struct hash<origin::vertex_handle>
{
  std::size_t
  operator() (origin::vertex_handle x) const
  {
    return x.hash ();
  }
};

template <typename E> struct hash<origin::multi_edge_handle<E> >
{
  std::size_t
  operator() (const origin::multi_edge_handle<E> &h)
  {
    return h.hash ();
  }
};
