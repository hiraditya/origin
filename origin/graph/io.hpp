...
template <typename G> struct vertex_list_printer
{
...
vertex_list (const G &g)
{
  return vertex_list_printer<G> (g);
}

// Support the printing of edge lists.
template <typename G> struct edge_list_printer
{
...
edge_list (const G &g)
{
  return edge_list_printer<G> (g);
}

// Supports the printing of edge tuples.
template <typename G> struct edge_printer
{
...
edge (const G &g, Edge<G> e)
{
  return edge_printer<G> (g, e);
}
