...
public:
  ...
  using queue_type = std::priority_queue<std::size_t, std::vector<std::size_t>,
                                         std::greater<size_t> >;
...
private:
  ...
  void append_nonempty (std::size_t n, Args &&... x);
...
template <typename T> inline auto pool<T>::data () const -> const list_type &
{
  return nodes_;
}

// Returns the free index list.
template <typename T> inline auto pool<T>::free () const -> const queue_type &
{
  return free_;
}
...
// previously erased.
template <typename T> inline T &pool<T>::operator[] (std::size_t n)
{
  assert (alive (n));
  return nodes_[n].get ();
}

template <typename T> inline const T &pool<T>::operator[] (std::size_t n) const
{
  assert (alive (n));
  return nodes_[n].get ();
}
...
template <typename T> pool_node<T>::pool_node () : prev (npos), next (npos) {}
...
template <typename T> pool_node<T>::~pool_node () { destroy (); }

template <typename T>
inline bool
pool_node<T>::valid () const
{
  return next != npos;
}
...
public:
  ...
  using node_type
      = If<Const<T> (), const pool_node<value_type>, pool_node<value_type> >;
...
template <typename T>
inline pool_iterator<T>::pool_iterator ()
    : p_ (nullptr), i_ (-1)
{
}

template <typename T>
inline pool_iterator<T>::pool_iterator (pool_type *p, std::size_t i)
    : p_ (p), i_ (i)
{
}

template <typename T> inline T &pool_iterator<T>::operator* () const
{
...
template <typename T> inline pool_iterator<T> &pool_iterator<T>::operator++ ()
{
  incr ();
  return *this;
}
...
template <typename T>
inline pool_iterator<T> pool_iterator<T>::operator++ (int)
{
